...
import { googleSignIn, logout, getAccessToken, emailSignIn, emailSignUp } from '../lib/firebase';
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (result) {
        ...
        setDriveNotification({ message: 'Welcome to your premium Workspace!', type: 'success' });
      }
    } catch (err: any) {
      console.error('Portal Login Error:', err);
      let friendlyError = err.message || 'Failed to authenticate. Please try again.';
      ...
      if (isPopupClosed) {
        friendlyError = "Google login popup was closed. This usually happens if Google's Advanced Protection Program (which blocks unverified applets) closed the window, if a browser popup blocker intercepted it, or if the sign-in was cancelled. 💡 Highly Recommended: Click 'Enter VIP Local Session' next to the login button to bypass this Google restriction entirely and access the portal offline!";
      } else if (err.message && typeof err.message === 'string' && (err.message.includes('policy_enforced') || err.message.includes('Access blocked'))) {
        friendlyError = "Access blocked by Google Account Security (Advanced Protection / 'policy_enforced'). 💡 Do not worry! Click 'Enter VIP Local Session' to bypass this check and download all templates and schedules directly to your device!";
      }
  ...
  const handleEmailAuthSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authMode === 'email-signup') {
        loggedUser = await emailSignUp(email, password);
        setDriveNotification({
          message: "Welcome! Your ADHD VIP account has been successfully created.",
          type: "success"
        });
      } else {
        loggedUser = await emailSignIn(email, password);
      }
      ...
      if (err.code === 'auth/operation-not-allowed') {
        ...
        friendlyError = "This email is already in use. Please select Sign In instead or use a different email.";
        ...
        <AnimatePresence mode="wait">
          {!user ? (
            ...
            >
              ...
              <div className="flex bg-neutral-900/60 p-1 rounded-xl border border-neutral-800/80 mb-6 max-w-md w-full">
                <button
                  onClick={() => { setAuthMode('google'); setError(null); }}
                  className={`flex-1 text-center py-2 px-3 rounded-lg text-xs font-bold font-display transition-all cursor-pointer ${
                    authMode === 'google' 
              ...
              {/* Educational Warning about Google Advanced Protection Policy Block (only visible for google flow) */}
              {authMode === 'google' && (
                <div className="w-full bg-amber-500/5 border border-amber-500/20 rounded-2xl p-4 mb-6 text-left text-xs text-neutral-300 leading-relaxed">
                  <div className="flex items-start gap-3">
                    <div className="p-1 rounded bg-amber-500/10 text-amber-400 shrink-0 mt-0.5">
                      <Sparkles className="w-4 h-4 animate-pulse" />
                    </div>
                    <div>
                      <h4 className="font-bold text-amber-400 mb-1 font-display">Using a Workspace Account or Google Advanced Protection?</h4>
                      <p className="text-neutral-400 leading-normal text-[11px]">
                        Because Google's <span className="text-amber-300 font-semibold">Advanced Protection Program</span> strictly blocks unverified third-party applets from requesting Google Drive sync scopes (Google's "policy_enforced" rule), you might encounter an access-blocked message. 
                      </p>
                      <p className="text-neutral-400 leading-normal text-[11px] mt-1.5">
                        No problem! Switch to <span className="text-amber-300 font-semibold">"Email Sign In / Create Account"</span> above, or click <span className="text-amber-300 font-semibold">"Enter VIP Local Session"</span> below to instantly bypass the block and unlock the entire portal!
                      </p>
                    </div>
                  </div>
                </div>
              ...
              {authMode === 'google' ? (
                /* High-fidelity Exclusive GSI Sign In Button Container */
                <div className="flex flex-col sm:flex-row gap-4 w-full items-center justify-center">
                  <button 
                    onClick={handleLogin}
                    disabled={isLoggingIn}
                    className="relative overflow-hidden w-full sm:w-auto bg-black border-2 border-amber-500/40 hover:border-amber-500 active:scale-98 py-3 px-6 rounded-xl text-white font-display font-bold flex items-center justify-center gap-3.5 transition-all duration-300 shadow-[0_0_25px_rgba(212,175,55,0.15)] group cursor-pointer disabled:opacity-50"
                  >
                    {/* Glowing Golden Shimmer Sweep */}
                    <div className="absolute inset-0 pointer-events-none overflow-hidden">
                      <motion.div
                        ...
                        className="absolute inset-0 w-[200%] h-full bg-gradient-to-r from-transparent via-amber-400/10 to-transparent skew-x-12"
                      />
                    </div>
                    ...
                    <span className="font-display tracking-tight group-hover:text-amber-400 transition-colors text-xs sm:text-sm">
                      {isLoggingIn ? 'Unlocking Workspace...' : 'Secure Log In with Google'}
                    </span>
                  </button>

                  <button 
                    onClick={() => onLoginSuccess({ displayName: "VIP Local Member", email: "adhd.VIP@local.dev", photoURL: null }, "local-session")}
                    className="w-full sm:w-auto bg-neutral-900 hover:bg-neutral-800 border-2 border-neutral-800 hover:border-amber-500/30 active:scale-98 py-3 px-5 rounded-xl text-amber-400 font-display font-bold flex items-center justify-center gap-2 transition-all duration-300 cursor-pointer text-xs sm:text-sm"
                  >
                    <Sparkles className="w-4 h-4 text-amber-400 animate-pulse" />
                    <span>Enter VIP Local Session</span>
                  </button>
                </div>
              ) : (
                /* High-fidelity Exclusive Email & Password Form */
                <form onSubmit={handleEmailAuthSubmit} className="w-full max-w-md bg-neutral-900/40 border border-neutral-800/80 p-6 rounded-2xl text-left shadow-[0_4px_30px_rgba(0,0,0,0.4)]">
                  <h3 className="text-white font-display font-bold text-base mb-4 flex items-center gap-2">
                    <Lock className="w-4 h-4 text-amber-400" />
                    <span>{authMode === 'email-signin' ? 'Sign In with Email' : 'Create Premium Email Account'}</span>
                  </h3>
                  ...
                  <div className="flex flex-col sm:flex-row gap-3">
                    <button
                      type="submit"
                      disabled={isLoggingIn}
                      className="flex-1 bg-gold-gradient text-neutral-950 font-display font-bold py-2.5 px-4 rounded-xl hover:opacity-90 active:scale-98 transition-all flex items-center justify-center gap-2 cursor-pointer text-xs disabled:opacity-50"
                    >
                      {isLoggingIn ? 'Verifying...' : authMode === 'email-signin' ? 'Sign In' : 'Create Account'}
                    </button>
                    <button
                      type="button"
                      onClick={() => onLoginSuccess({ displayName: "VIP Local Member", email: "adhd.VIP@local.dev", photoURL: null }, "local-session")}
                ...
                <div className="bg-black/40 border border-amber-500/10 p-4 rounded-2xl mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] font-mono text-amber-500/80">Premium Sync Active</p>
                    </div>
                  </div>
                </div>
